...
fun scrubAddresses(text: String?): String? {
    ...
    var s = text
    s = s.replace(Regex("https?://\\S+", RegexOption.IGNORE_CASE), "сервер")
    // домены вида name.tld (uz/com/net/org/ru/io…)
    s = s.replace(Regex("\\b[a-z0-9-]+(?:\\.[a-z0-9-]+)+\\.[a-z]{2,}\\b", RegexOption.IGNORE_CASE), "сервер")
    s = s.replace(Regex("\\b[a-z0-9-]+\\.[a-z]{2,}\\b", RegexOption.IGNORE_CASE), "сервер")
    // IPv4 с необязательным портом и ведущим слэшем
    s = s.replace(Regex("/?\\b(?:\\d{1,3}\\.){3}\\d{1,3}(?::\\d+)?\\b"), "сервер")
    ...
    s = s.replace(Regex("(?:сервер[\\s/:]+){1,}сервер", RegexOption.IGNORE_CASE), "сервер")
...
fun friendlyNetworkError(error: Throwable): String? = when (error) {
    ...
    is IOException -> {
        ...
        when {
            "unable to resolve host" in m -> "Нет связи с сервером — проверьте интернет"
            "failed to connect" in m || "connection refused" in m -> "Сервер недоступен"
            "timeout" in m -> "Сервер не ответил вовремя"
            "not verified" in m || "hostname" in m -> "Не удалось установить защищённое соединение"
            else -> null
        }
    }
    else -> null
